...
(function(FILTER, undef){
    ...
    // geometric maps
    var
        /*trivialMap=function(im, w, h) { return im; },*/
        
        genericMap=function(im, w, h) {
        ...
        twirlMap=function(im, w, h)  {
            ...
            while (i<imLen)
            {
                ...
                // Opera (Chrome ??), distorts the image if not set these here explicitly
                // it seems dst=new IMG(im), does NOT copy the original array (??)
                // using dst=new IMG(imLen), which creates a blank array,
                // and setting the other values here, explicitly
                /*else
                {
                    dst[i]=im[i];   dst[i+1]=im[i+1];
                    dst[i+2]=im[i+2];  dst[i+3]=im[i+3];
                }*/
                i+=4; x++; if (x>=w) { x=0; y++; }
            }
            return im;
    ...
    //
    // GeometricMapFilter
    var GeometricMapFilter = FILTER.GeometricMapFilter = FILTER.Extends( FILTER.Filter,
    {
